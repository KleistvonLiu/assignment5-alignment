...
from cs336_alignment.alignment_data import load_simple_safety_tests
...
def generate_safety_outputs(args: argparse.Namespace) -> dict[str, Any]:
    examples = load_simple_safety_tests(args.data_path)
    if args.limit is not None:
        examples = examples[: args.limit]

    prompt_style = PromptStyle(args.prompt_style)
    prompts = [render_prompt(prompt_style, example["prompts_final"]) for example in examples]
...
def main() -> None:
    summary = generate_safety_outputs(parse_args())
    logger.info("SimpleSafetyTests generation summary: %s", json.dumps(summary, indent=2))
